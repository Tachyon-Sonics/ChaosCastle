...
public class ChaosCreator {
    ...
    private void MakeAlienV(ChaosBase.Obj alien) {
        ...
        if (alien.moveSeq < ChaosBase.Period)
            ...
            px = 0;
        chaosActions.SetObjLoc(alien, px, 128, 20, 20);
    ...
    private void MakeFour(ChaosBase.Obj four) {
        ...
        if (four.moveSeq < ChaosBase.Period)
            ...
            py = 160;
        chaosActions.SetObjLoc(four, 200, py, 24, 24);
    ...
    private void MakeQuad(ChaosBase.Obj quad) {
        chaosActions.SetObjLoc(quad, 5, 185, 22, 22);
    ...
    private void MakeCreatorR(ChaosBase.Obj creator) {
        chaosActions.SetObjLoc(creator, 96, 128, 32, 32);
    ...
    private void MakeCreatorC(ChaosBase.Obj creator) {
        if (creator.moveSeq >= ChaosBase.Period * 3) {
            chaosActions.SetObjLoc(creator, 128, 132, 28, 28);
            chaosActions.SetObjRect(creator, 0, 0, 28, 28);
        } else {
            chaosActions.SetObjLoc(creator, 156, 132, 28, 28);
        }
    }
    ...
    private void MakeCircle(ChaosBase.Obj circle) {
        if (circle.shapeSeq == 0)
            chaosActions.SetObjLoc(circle, 64, 96, 32, 32);
        else
            chaosActions.SetObjLoc(circle, 224, 132, 32, 32);
        chaosActions.SetObjRect(circle, 1, 1, 31, 31);
    }

    private final ChaosBase.MakeProc MakeCircle_ref = this::MakeCircle;

    private void FireMeteorite(int px, int py, int mvx, int mvy, int ns) {
        // VAR
        ChaosBase.Obj mt = null;

        mt = chaosActions.CreateObj(Anims.DEADOBJ, ChaosDObj.doMeteor, px, py, ns, 1);
    ...
    private void MakeController(ChaosBase.Obj ctrl) {
            ...
            if (chaosBase.zone != Zone.Castle) {
                if (ctrl.stat == 0) {
                    ...
                    if (trigo.RND() % 2 == 0) {
                        ...
                        px = chaosGraphics.gameWidth + ChaosGraphics.BW;
                        nvx = -800;
                    }
                    md = (chaosGraphics.gameHeight / 2);
                    py = trigo.RND() % md;
                    hu = chaosActions.CreateObj(Anims.ALIEN2, cHurryUp, px, py, 1, 100);
                    chaosActions.SetObjVXY(hu, nvx, 0);
        ...
        if (ctrl.shapeSeq == 0) {
            if (chaosBase.water) {
                wa = chaosBase.GetAnimAttr(Anims.DEADOBJ, ChaosDObj.doWave);
                if (wa.nbObj < 12) {
                    angle = trigo.RND() % 360;
                    px = trigo.COS(angle) / 16 * ChaosGraphics.PW / 64;
                    py = trigo.SIN(angle) / 16 * ChaosGraphics.PH / 64;
                    ...
                    mvx = -(mvx * speed / 1024);
                    mvy = -(mvy * speed / 1024);
                    wv = chaosActions.CreateObj(Anims.DEADOBJ, ChaosDObj.doWave, px, py, trigo.RND() % 15, 1);
                    chaosActions.SetObjVXY(wv, (int) mvx, (int) mvy);
                }
            ...
            if ((chaosBase.difficulty >= 3) && ((chaosBase.zone == Zone.Chaos) || ((chaosBase.zone == Zone.Family) && (chaosBase.level[Zone.Family.ordinal()] == 6)))) {
                ...
                if (chaosBase.level[Zone.Castle.ordinal()] > 1) {
                    ...
                    if ((nb == 0) || (chaosBase.level[Zone.Chaos.ordinal()] % 33 == 0) || ((chaosBase.difficulty >= 6) && (chaosBase.level[Zone.Chaos.ordinal()] % 10 == 0))) {
                        FireMeteorite(px, py, (int) mvx, (int) mvy, ChaosDObj.domBig);
                        mf = true;
                    }
                    if (chaosBase.difficulty >= 5) {
                        md = chaosBase.level[Zone.Chaos.ordinal()] % 13;
                        if (md % 8 == 0) {
                            px = ChaosGraphics.PW - px;
                            mvx = -mvx;
                            py = ChaosGraphics.PH - py;
                            mvy = -mvy;
                            mf = true;
                            FireMeteorite(px, py, (int) mvx, (int) mvy, ChaosDObj.domMedium);
                        }
                    }
                    ...
                    if ((md == 9) || ((chaosBase.difficulty >= 4) && (md == 3))) {
                        if (trigo.RND() % 2 == 0) {
                            ...
                            py -= mvx / 64;
                            if (mf && (trigo.RND() % 8 == 0)) {
                                FireMeteorite(px, py, (int) mvx, (int) mvy, ChaosDObj.domSmall);
                                px -= mvy / 24;
                                py += mvx / 32;
                            }
                        }
                ...
                if ((chaosBase.difficulty >= 7) && ((chaosBase.level[Zone.Chaos.ordinal()] % 7 == 0) || (chaosBase.level[Zone.Chaos.ordinal()] == 100))) {
                    ...
                    px = ChaosGraphics.PW - px;
                    mvx = -mvx;
                    FireMeteorite(px, py, (int) mvx, (int) mvy, ChaosDObj.domStone);
                }
    ...
    private void MakeChief(ChaosBase.Obj chief) {
        chaosActions.SetObjLoc(chief, 128, 72, 16, 16);
    ...
    private void MakeABox(ChaosBase.Obj abox) {
        chaosActions.SetObjLoc(abox, 0, 180, 32, 32);
    ...
    private void MakeNest(ChaosBase.Obj nest) {
        chaosActions.SetObjLoc(nest, 184, 132, 28, 28);
    ...
    private void MakeGrid(ChaosBase.Obj grid) {
        chaosActions.SetObjLoc(grid, 96, 96, 32, 32);
    ...
    private void MakeMissile(ChaosBase.Obj missile) {
        ...
        if (missile.stat == 0)
            ...
            px = 64;
        chaosActions.SetObjLoc(missile, px, 100, 12, 24);
    ...
    private void MakePopUp(ChaosBase.Obj popup) {
        ...
        if (popup.stat == 2)
            ...
            sz = 12;
        chaosActions.SetObjLoc(popup, 52, 200, sz, sz);
    ...
    private void MakeGhost(ChaosBase.Obj ghost) {
        chaosActions.SetObjLoc(ghost, 120, 180, 20, 20);
    ...
    private void ResetFour(ChaosBase.Obj four) {
        ...
        four.dvx = trigo.RND() % 2048;
        four.dvx -= 1024;
        four.dvy = trigo.RND() % 2048;
        four.dvy -= 1024;
        MakeFour(four);
    }
    ...
    private void ResetQuad(ChaosBase.Obj quad) {
        ...
        quad.dvx = trigo.RND() % 2048;
        quad.dvx -= 1024;
        quad.dvy = trigo.RND() % 2048;
        quad.dvy -= 1024;
        MakeQuad(quad);
    }
    ...
    private void ResetCircle(ChaosBase.Obj circle) {
        ...
        circle.dvx = trigo.RND() % 4096;
        circle.dvx -= 2048;
        circle.dvy = trigo.RND() % 4096;
        circle.dvy -= 2048;
    ...
    private void ResetGrid(ChaosBase.Obj grid) {
        ...
        grid.dvx = trigo.RND() % 1024 + 1024;
        grid.ax = 0;
        grid.dvy = trigo.RND() % 1024 + 1024;
        grid.ay = 0;
    ...
    private void FireMissileV(ChaosBase.Obj src, int sk) {
        // VAR
        ...
        nvx = (int) (rx * 2048 / rl);
        nvy = (int) (ry * 2048 / rl);
        missile = chaosActions.CreateObj(Anims.MISSILE, ChaosMissile.mAlien1, px.get(), py.get(), sk, 8 + sk * 2);
        chaosActions.SetObjVXY(missile, nvx, nvy);
    }

    private void FireMissileA(ChaosBase.Obj src, int ox, int oy, boolean snd) {
        // VAR
        ChaosBase.Obj missile = null;
        long rx = 0L;
        long ry = 0L;
        ...
        missile = chaosActions.CreateObj(Anims.MISSILE, ChaosMissile.mAlien1, px.get(), py.get(), sk, 10);
        chaosActions.SetObjVXY(missile, nvx, nvy);
        missile.ax = nax;
        missile.ay = nay;
    }
    ...
    private void MoveAlienAV(ChaosBase.Obj alien) {
        ...
        chaosActions.LimitSpeed(alien, 1024);
        ...
        if ((alien.moveSeq >= ChaosBase.Period) && (chaosBase.step > alien.moveSeq - ChaosBase.Period)) {
            ...
            if (alien.shapeSeq == 0) {
                if (alien.subKind == cAlienV) {
                    ...
                    FireMissileV(alien, ChaosMissile.mBlue);
    ...
    private void MoveCreatorR(ChaosBase.Obj creator) {
        ...
        chaosActions.AvoidBounds(creator, 4);
        chaosActions.AvoidBackground(creator, 4);
    ...
    private void MoveCreatorC(ChaosBase.Obj creator) {
        ...
        if (chaosBase.step > creator.moveSeq) {
            ...
            alien = chaosActions.CreateObj(nKind, sKind, px.get(), py.get(), 0, chaosBase.pLife * 3);
            angle = creator.stat * 90 + creator.shapeSeq;
            chaosActions.SetObjVXY(alien, trigo.COS(angle) * 3, trigo.SIN(angle) * 3);
            if (creator.stat == 0) {
    ...
    private void BoumMissile(ChaosBase.Obj obj, boolean force) {
        ...
        angle = trigo.RND() % 32;
        for (c = 1; c <= 20; c++) {
            nvx = trigo.COS(angle) * 3 / 2;
            nvy = trigo.SIN(angle) * 3 / 2;
            if (obj.subKind == cPopUp)
                sk = c % 5;
            else
                sk = ChaosMissile.mBlue;
            missile = chaosActions.CreateObj(Anims.MISSILE, ChaosMissile.mAlien1, px.get(), py.get(), sk, 9);
            chaosActions.SetObjVXY(missile, nvx, nvy);
            angle += 18;
        }
    }
    ...
    private void MoveCircle(ChaosBase.Obj circle) {
        ...
        chaosActions.AvoidBounds(circle, 3);
        chaosActions.AvoidBackground(circle, 3);
        ...
        if (chaosActions.OutOfBounds(hu)) {
            ...
            if (chaosBase.step > hu.moveSeq) {
                ...
                if (py.get() > chaosGraphics.gameHeight / 2)
                    ...
                    nay = 40;
                missile = chaosActions.CreateObj(Anims.MISSILE, ChaosMissile.mAlien1, px.get(), py.get(), ChaosMissile.mGreen, 8);
                chaosActions.SetObjAXY(missile, 0, nay);
    ...
    private void MoveChief(ChaosBase.Obj chief) {
        ...
        nvx = 5 + chaosBase.difficulty;
        speed = nvx;
        chaosActions.LimitSpeed(chief, nvx * 256);
        chaosActions.AvoidBounds(chief, 0);
        ...
        if (chief.stat == 0) {
            ...
            if (chaosBase.step > chief.shapeSeq) {
                ...
                if (rnd == 0) {
                    FireMissileV(chief, trigo.RND() % 4);
                } else if (rnd == 1) {
                    ...
                    _new = chaosActions.CreateObj(nKind, sKind, mx.get(), my.get(), nStat, chaosBase.pLife);
                    chaosActions.SetObjVXY(_new, nvx, 0);
                }
            }
            ...
            if (dl != 0) {
                chief.dvx = (int) (lx * 128 / dl * speed);
                chief.dvy = (int) (ly * 128 / dl * speed);
            }
            if (dl < 56) {
                chief.dvx = -chief.dvx;
                chief.dvy = -chief.dvy;
            ...
            if (bg && (Math.abs(chief.vx) < 400) && (Math.abs(chief.vy) < 400)) {
                ...
                if (chief.moveSeq != 0) {
                    { // WITH
                        ...
                        if (_chiefData.dir)
                            _chiefData.angle = (_chiefData.angle - 90) % 360;
                        else
                            _chiefData.angle = (_chiefData.angle + 90) % 360;
                        _chiefData.dstx = mx.get() / ChaosGraphics.BW;
                        _chiefData.dsty = my.get() / ChaosGraphics.BH;
                        _chiefData.crx = _chiefData.dstx;
                        _chiefData.cry = _chiefData.dsty;
                        _chiefData.tox = _chiefData.crx;
                        _chiefData.toy = _chiefData.cry;
                        px.set(px.get() / ChaosGraphics.BW);
                        py.set(py.get() / ChaosGraphics.BH);
                        while (true) {
            ...
            { // WITH
                ...
                chaosActions.GetCenter(chief, mx, my);
                px.set(_chiefData.tox * ChaosGraphics.BW + ChaosGraphics.BW / 2);
                py.set(_chiefData.toy * ChaosGraphics.BH + ChaosGraphics.BH / 2);
                lx = px.get() - mx.get();
                ly = py.get() - my.get();
                ...
                if (Math.abs(dl) <= 16) {
                    ...
                    if (_chiefData.dir)
                        ...
                        _chiefData.angle = (_chiefData.angle - 90) % 360;
                    while (true) {
                        _chiefData.tox = _chiefData.crx + trigo.COS(_chiefData.angle) / 1024;
                        _chiefData.toy = _chiefData.cry + trigo.SIN(_chiefData.angle) / 1024;
    ...
    private void MoveABox(ChaosBase.Obj abox) {
        ...
        chaosActions.AvoidBounds(abox, 3);
        chaosActions.AvoidBackground(abox, 3);
    ...
    private void MoveGrid(ChaosBase.Obj grid) {
        ...
        ovx = (int) Math.abs(grid.dvx);
        ovy = (int) Math.abs(grid.dvy);
        chaosActions.UpdateXY(grid);
        chaosActions.AvoidBounds(grid, 4);
        chaosActions.AvoidBackground(grid, 4);
        grid.dvx = ovx;
        grid.dvy = ovy;
        if (grid.vx >= grid.dvx) {
            grid.ax = grid.shapeSeq;
            grid.ax = -grid.ax;
        } else if (-grid.vx >= grid.dvx) {
            grid.ax = grid.shapeSeq;
        }
        if (grid.vy >= grid.dvy) {
            grid.ay = grid.moveSeq;
            grid.ay = -grid.ay;
    ...
    private void MovePopUp(ChaosBase.Obj popup) {
        ...
        chaosActions.AvoidBackground(popup, 1);
        chaosActions.AvoidBounds(popup, 1);
        popup.dvx = 0;
        popup.dvy = 0;
        if (popup.stat == 2) {
            chaosActions.GetCenter(chaosBase.mainPlayer, px, py);
            ...
            if (chaosBase.step > popup.moveSeq) {
                if (popup.stat == 1) {
                    ...
                    for (angle = 0; angle <= 270; angle += 90) {
                        ...
                        missile = chaosActions.CreateObj(Anims.MISSILE, ChaosMissile.mAlien1, px.get(), py.get(), ChaosMissile.mRed, 10);
                        chaosActions.SetObjVXY(missile, nvx, nvy);
                    }
                    popup.stat = 0;
                    popup.moveSeq += ChaosBase.Period * 3 / 2;
    ...
    private void MoveGhost(ChaosBase.Obj ghost) {
        ...
        chaosActions.AvoidBackground(ghost, 0);
        chaosActions.GetCenter(chaosBase.mainPlayer, px, py);
        chaosActions.GetCenter(ghost, mx, my);
        speed = (800 + chaosBase.difficulty * 100);
        ghost.dvx = trigo.SGN(px.get() - mx.get()) * speed;
        ghost.dvy = trigo.SGN(py.get() - my.get()) * speed;
        if (ghost.moveSeq == 0)
    ...
    private void AieCircle(ChaosBase.Obj circle, ChaosBase.Obj src, /* VAR+WRT */ Runtime.IRef<Integer> hit, /* VAR+WRT */ Runtime.IRef<Integer> fire) {
        ...
        if ((circle.life == 0) && (chaosBase.zone == Zone.Chaos))
            chaosActions.PopMessage(Runtime.castToRef(languages.ADL("Get the money"), String.class), ChaosActions.moneyPos, 2);
        circle.life++;
        circle.hitSubLife = 0;
    ...
    private void DieCreatorR(ChaosBase.Obj creator) {
        ...
        if (chaosBase.difficulty >= 5) {
            ...
            while (c > 0) {
                ...
                chaosActions.SetObjVXY(diese, trigo.COS(angle) * 3, trigo.SIN(angle) * 3);
                angle += 360 / cnt;
                c--;
            }
        }
    ...
    private void DieCircle(ChaosBase.Obj circle) {
        ...
        if (chaosBase.difficulty >= 9) {
            ...
            while (c > 0) {
                chief = chaosActions.CreateObj(Anims.ALIEN2, cChief, px.get(), py.get(), 0, chaosBase.pLife * 4);
                chaosActions.SetObjVXY(chief, trigo.COS(angle) * 4, trigo.SIN(angle) * 4);
                angle += 360 / cnt;
                c--;
            }
        }
    ...
    private void DieNest(ChaosBase.Obj nest) {
        ...
        if (bullets < 100 + trigo.RND() % 200) {
            chaosActions.GetCenter(nest, px, py);
            bonus = chaosActions.CreateObj(Anims.BONUS, ChaosBonus.TimedBonus, px.get(), py.get(), ChaosBonus.tbBullet, 1);
        }
    ...
    private void InitParams() {
        ...
        chaosSounds.SetEffect(hurryupEffect[0], chaosSounds.soundList[SoundList.sHurryUp.ordinal()], 0, 0, 200, 1);
        chaosSounds.SetEffect(missileEffect[0], chaosSounds.soundList[SoundList.sMissile.ordinal()], 0, 0, 110, 4);
        chaosSounds.SetEffect(aieCircleEffect[0], chaosSounds.soundList[SoundList.sGong.ordinal()], 0, 16726, 200, 7);
        chaosSounds.SetEffect(dieCircleEffect[0], chaosSounds.soundList[SoundList.sCasserole.ordinal()], 0, 0, 250, 9);
        chaosSounds.SetEffect(aieAlienEffect[0], chaosSounds.soundList[SoundList.sCymbale.ordinal()], 0, 0, 140, 1);
        chaosSounds.SetEffect(aieHUEffect[0], chaosSounds.soundList[SoundList.sHurryUp.ordinal()], 0, 16726, 180, 1);
        chaosSounds.SetEffect(dieHUEffect[0], chaosSounds.soundList[SoundList.sCannon.ordinal()], 4500, 0, 170, 4);
        chaosSounds.SetEffect(aieCreatorEffect[0], chaosSounds.soundList[SoundList.sHHat.ordinal()], 0, 4181, 180, 3);
        chaosSounds.SetEffect(aieChiefEffect[0], chaosSounds.soundList[SoundList.sPoubelle.ordinal()], 0, 0, 220, 5);
        chaosSounds.SetEffect(dieAlienVEffect[0], chaosSounds.soundList[SoundList.aCrash.ordinal()], 0, 0, 180, 4);
        chaosSounds.SetEffect(dieAlienVEffect[1], chaosSounds.soundList[SoundList.wCrash.ordinal()], 582, 0, 180, 3);
        chaosSounds.SetEffect(dieAlienAEffect[0], chaosSounds.soundList[SoundList.aCrash.ordinal()], 0, 8860, 180, 4);
        chaosSounds.SetEffect(dieAlienAEffect[1], chaosSounds.soundList[SoundList.wCrash.ordinal()], 582, 7451, 180, 3);
        for (c = 2; c <= 16; c++) {
            v = (17 - c) * 11;
            chaosSounds.SetEffect(dieAlienAEffect[c], chaosSounds.nulSound, 582, 8860 - (c % 2) * 1409, v, 3);
            v = v * (c % 2);
            chaosSounds.SetEffect(dieAlienVEffect[c], chaosSounds.nulSound, 582, 0, v, 3);
        }
        for (c = 0; c <= 23; c++) {
            v = Math.abs(c % 4 - 2) + 1;
            chaosSounds.SetEffect(dieCreatorREffect[c], chaosSounds.soundList[SoundList.wWhite.ordinal()], 700, 8363 + v * 2788, (24 - c) * v * 7 / 2, 5);
            chaosSounds.SetEffect(dieNestEffect[c], chaosSounds.soundList[SoundList.wNoise.ordinal()], 700, 8363 + v * 2788, (24 - c) * v * 3, 5);
        }
        v = 0;
        for (c = 0; c <= 7; c++) {
            chaosSounds.SetEffect(dieCreatorCEffect[c], chaosSounds.soundList[SoundList.wWhite.ordinal()], 0, 5575 + 995 * v, (8 - c) * 24, 5);
            v = (v * 9 + 5) % 8;
        }
        chaosSounds.SetEffect(dieFourEffect[0], chaosSounds.soundList[SoundList.aCrash.ordinal()], 0, 0, 192, 4);
        for (c = 1; c <= 8; c++) {
            chaosSounds.SetEffect(dieFourEffect[c], chaosSounds.soundList[SoundList.wCrash.ordinal()], 1500, 8363, (9 - c) * 24, 4);
        }
        for (c = 0; c <= 11; c++) {
            v = c % 3;
            if (v == 0)
            ...
            chaosSounds.SetEffect(dieABEffect[c], chaosSounds.soundList[snd.ordinal()], 836, 8363, (12 - c) * 16, 5);
        }
        chaosSounds.SetEffect(circleFireEffect[0], chaosSounds.soundList[SoundList.sPouf.ordinal()], 5500, 0, 190, 6);
        chaosSounds.SetEffect(popupEffect[0], chaosSounds.soundList[SoundList.wVoice.ordinal()], 929, 8363, 100, 4);
        chaosSounds.SetEffect(popupEffect[1], chaosSounds.soundList[SoundList.wVoice.ordinal()], 1394, 12544, 100, 4);
        chaosSounds.SetEffect(popupEffect[2], chaosSounds.soundList[SoundList.wVoice.ordinal()], 1162, 10454, 100, 4);
        chaosSounds.SetEffect(diePopupEffect[0], chaosSounds.soundList[SoundList.sCymbale.ordinal()], 0, 4181, 100, 4);
        chaosSounds.SetEffect(aieGridEffect[0], chaosSounds.soundList[SoundList.wCrash.ordinal()], 836, 16726, 130, 2);
        chaosSounds.SetEffect(aieGridEffect[1], chaosSounds.soundList[SoundList.wCrash.ordinal()], 627, 12544, 130, 2);
        chaosSounds.SetEffect(aieGridEffect[2], chaosSounds.soundList[SoundList.wCrash.ordinal()], 523, 10454, 130, 2);
        chaosSounds.SetEffect(aieGridEffect[3], chaosSounds.soundList[SoundList.wCrash.ordinal()], 418, 8363, 130, 2);
        chaosSounds.SetEffect(aieGhostEffect[0], chaosSounds.soundList[SoundList.sHurryUp.ordinal()], 0, 4181, 100, 1);
        chaosSounds.SetEffect(dieGhostEffect[0], chaosSounds.soundList[SoundList.wWhite.ordinal()], 0, 0, 160, 4);
        ...
        attr.dieSKCount = 1;
        attr.dieStone = ChaosBase.FlameMult * 4 + 20;
        ...
        attr.dieStKinds = EnumSet.of(Stones.stCE);
        attr.dieSKCount = 1;
        attr.dieStone = ChaosBase.FlameMult * 4 + 9;
        attr.dieStStyle = ChaosBase.slowStyle;
        attr.basicType = BasicTypes.Vegetal;
        attr.toKill = true;
        memory.AddTail(chaosBase.attrList[Anims.ALIEN2.ordinal()], attr.node);
